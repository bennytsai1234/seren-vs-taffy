...
export async function GET() {
  try {
    ...
    return NextResponse.json({ seren: 0, taffy: 0 });
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    const visitorId = cookieStore.get("visitor_id")?.value || Math.random().toString(36).slice(2);
    // simple cookie check — already voted in this session
